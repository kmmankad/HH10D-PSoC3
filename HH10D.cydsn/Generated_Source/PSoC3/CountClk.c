...
void CountClk_Start(void) 
{
    ...
    CountClk_CLKEN |= CountClk_CLKEN_MASK;
}
...
void CountClk_Stop(void) 
{
	...
	CYDEV_CHIP_REVISION_USED == CYDEV_CHIP_REVISION_5A_ES1))
/*******************************************************************************
* Function Name: CountClk_StopBlock
********************************************************************************
* Summary:
...
{
    if (CountClk_CLKEN & CountClk_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = CountClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CountClk_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(CountClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CountClk_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: CountClk_StandbyPower
********************************************************************************
* Summary:
...
void CountClk_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        CountClk_CLKSTBY &= ~CountClk_CLKSTBY_MASK;
    }
    else
    {
        CountClk_CLKSTBY |= CountClk_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: CountClk_SetDividerRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void CountClk_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    uint8 enabled;

    uint8 currSrc = CountClk_GetSourceRegister();
    uint16 oldDivider = CountClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CountClk_CLKEN & CountClk_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(CountClk_DIV_PTR, clkDivider);
                CountClk_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
...
{
    return CY_GET_REG16(CountClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: CountClk_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void CountClk_SetModeRegister(uint8 clkMode) 
{
    CountClk_MOD_SRC |= clkMode & CountClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: CountClk_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void CountClk_ClearModeRegister(uint8 clkMode) 
{
    CountClk_MOD_SRC &= ~clkMode | ~CountClk_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: CountClk_GetModeRegister
********************************************************************************
* Summary:
...
{
    return CountClk_MOD_SRC & CountClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: CountClk_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = CountClk_GetDividerRegister();
    uint8 oldSrc = CountClk_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        CountClk_MOD_SRC |= CYCLK_SSS;
        CountClk_MOD_SRC =
            (CountClk_MOD_SRC & ~CountClk_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CountClk_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return CountClk_MOD_SRC & CountClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CountClk_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    CountClk_PHASE = clkPhase & CountClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CountClk_GetPhase
********************************************************************************
* Summary:
...
{
    return CountClk_PHASE & CountClk_PHASE_MASK;
}
